...
NONADJUDICABLE_RECIPIENT_PATTERNS: List[Tuple[str, str]] = [
    ...
    (r"\bAVAILABLE\s+UPON\s+REQUEST\b", "available_upon_request_placeholder"),
    (r"\bNETWORK\s+OF\s+(?:OVER\s+)?\d+\s+(AGENCIES|ORGANIZATIONS|RECIPIENTS|ENTITIES)\b", "network_of_recipients_placeholder"),
    (r"\b\d+\s+(?:TO\s+)?\d+\s+DIFFERENT\s+(ORGANIZATIONS|AGENCIES|RECIPIENTS|ENTITIES)\b", "different_recipients_placeholder"),
    (r"\bDIFFERENT\s+(ORGANIZATIONS|AGENCIES|RECIPIENTS|ENTITIES)\b", "different_recipients_placeholder"),
    (r"\bGRANTS?\s+MADE\s+TO\s+VARIOUS\b.*\b(ENTITIES|ORGANIZATIONS|CHARITIES|RECIPIENTS|FOUNDATIONS|NONPROFITS|NON\s+PROFITS)\b", "various_recipients_placeholder"),
    (r"\bDESIGNATIONS?\s+TO\s+OTHER\b.*\b(AGENCIES|UNITED\s+WAYS|UWS|ORGANIZATIONS|CHARITIES|ENTITIES)\b", "designations_to_other_recipients_placeholder"),
    (r"\b(?:\d+\s+)?PARTNER\s+CHARITY\s+ORGANIZATIONS\b", "partner_charity_organizations_placeholder"),
