...
def call_ollama(packet: Dict[str, Any], args: argparse.Namespace) -> Dict[str, Any]:
...
""".strip()
    payload: Dict[str, Any] = {
        ...
        "messages": [
            ...
            {"role": "user", "content": json.dumps(input_obj, ensure_ascii=False, sort_keys=True)},
        ],
        "stream": False,
    ...
    try:
        ...
        if args.write_failures_as_human_review:
            rec = {
                ...
                "worker": {"processed_at": now_stamp(), "model": args.model, "failure_recorded_as_human_review": True},
            }
            return sig, rec, err
        return sig, None, err
